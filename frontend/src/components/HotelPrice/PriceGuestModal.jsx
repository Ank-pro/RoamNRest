...
export const PriceGuestModal = ({
  ...
  function handleSubtract(type) {
    if (type === "adult") {
      dispatch(
        setGuest({ adults: adults > 1 ? adults - 1 : 1, childrens, pets })
      );
    } else if (type === "children") {
      dispatch(
        setGuest({ adults, childrens: childrens > 0 ? childrens - 1 : 0, pets })
      );
    } else {
  ...
  return (
    <div className="reserve-guest-modal" onClick={(e) => e.stopPropagation()}>
